...
//=>hello:moe
```
<http://jsfiddle.net/1181155148/n81dpwk8/1/>
...
//输出表达式结果，如果gender为0则返回字符串为<div>女</div>，反之为<div>男</div>
 ```
 ```js
class="timelist <%= is_overdue == 1? "expire" : "" %>"
...
var tpl = '<% _.each(obj, function (item) { %>';
tpl += '<div><%= item.name %></div>';
tpl += '<% }) %>';

$('#a').html(_.template(tpl)(data));
...
// zhuzhu
```
<http://jsfiddle.net/1181155148/38afLvwt/>
...
//=> "Hello Mustache!"
```
<http://jsfiddle.net/1181155148/nubpt57w/>
...
$("#a").html(compiled({epithet: "stooge"}));
//=> "Hello stooge"
```
<http://jsfiddle.net/1181155148/4qeanhva/>
